...
mod tests {
    ...
    fn test_logger_initialization() {
        ...
        let result = init_logger(Some(config));
        assert!(result.is_ok());
    ...
    fn test_log_cleanup() {
        ...
        for i in 0..15 {
            let file_path = log_dir.join(format!("test.{:04}.log", i));
            std::fs::write(&file_path, format!("Log content {}", i)).unwrap();
        }

        let result = cleanup_old_logs(&log_dir, "test", 10);
        assert!(result.is_ok());
        ...
        let remaining_files: Vec<_> = std::fs::read_dir(&log_dir)
            ...
            .filter_map(|entry| {
                ...
                if name_str.starts_with("test") && name_str.ends_with(".log") {
                    Some(())
                } else {
                    None
                }
