...
mod tests {
    ...
    fn create_test_upload_task() -> TaskType {
        TaskType::Upload {
            local_path: "/path/to/local/file.txt".to_string(),
    ...
    async fn test_task_crud_operations() {
        let task_store = TaskStore::new().unwrap();
        let session_id = "test_session_123".to_string();
        let task_type = create_test_upload_task();

        // Create task
        let task = task_store.create_task(session_id.clone(), task_type, 1024).unwrap();
        assert_eq!(task.status, TaskStatus::Pending);
        assert_eq!(task.total_size, 1024);

        // Update status
        task_store.update_task_status(&task.id, TaskStatus::InProgress, None).unwrap();
        let updated_task = task_store.get_task(&task.id).unwrap();
        assert_eq!(updated_task.status, TaskStatus::InProgress);
        assert!(updated_task.started_at.is_some());

        // Update progress
        task_store.update_task_progress(&task.id, 512, None).unwrap();
        let progress_task = task_store.get_task(&task.id).unwrap();
        assert_eq!(progress_task.transferred_size, 512);
        assert_eq!(progress_task.progress, 50.0);

        // Complete task
        task_store.update_task_status(&task.id, TaskStatus::Completed, None).unwrap();
        let completed_task = task_store.get_task(&task.id).unwrap();
        assert_eq!(completed_task.status, TaskStatus::Completed);
        assert!(completed_task.completed_at.is_some());

        // Delete task
        task_store.delete_task(&task.id).unwrap();
        assert!(task_store.get_task(&task.id).is_err());
    }

    #[tokio::test]
    async fn test_session_task_management() {
        let task_store = TaskStore::new().unwrap();
        let session_id1 = "session_1".to_string();
        ...
        // Verify session tasks
        let session1_tasks = task_store.get_session_tasks(&session_id1).unwrap();
        let session2_tasks = task_store.get_session_tasks(&session_id2).unwrap();

        assert_eq!(session1_tasks.len(), 3);
        assert_eq!(session2_tasks.len(), 2);

        // Test task filtering
        let pending_tasks = task_store.get_tasks_by_status(&session_id1, TaskStatus::Pending).unwrap();
        assert_eq!(pending_tasks.len(), 3);

        let unfinished_tasks = task_store.get_unfinished_tasks(&session_id1).unwrap();
        assert_eq!(unfinished_tasks.len(), 3);

        // Delete all tasks for session
        let deleted_count = task_store.delete_session_tasks(&session_id1).unwrap();
        assert_eq!(deleted_count, 3);

        let remaining_tasks = task_store.get_session_tasks(&session_id1).unwrap();
